...
def bind_epub_book(
    ...
    if book_cover and os.path.isfile(book_cover):
        ...
        cover_item = epub.EpubHtml(  # type:ignore
            title="Title Page",  # Changed from "Front Page"
    ...
    intro_item = epub.EpubHtml(  # type:ignore
        title="Synopsis",  # Changed from "Intro Page"
    ...
    for chapters in chapter_groups:
        ...
        for chapter in chapters:
            ...
            # Remove existing title tags from body if present
            body = body.replace(f"<h1>{title}</h1>", "", 1).strip()
            body = body.replace(f"<h2>{title}</h2>", "", 1).strip()
            
            chapter_content = f"""
            <h1>{title}</h1>
            <p style="text-align: center; margin: 1em 0; font-size: 1.2em;">━━━━━✧✧✧✧━━━━━</p>
            {body}
            """
    ...
    # Create custom navigation with CSS link
    nav = epub.EpubNav()
    nav.add_link(
        href=STYLE_FILE_NAME,
        rel="stylesheet",
